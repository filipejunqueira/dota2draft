...
def _load_kpi_parameters() -> Dict[str, Any]:
    ...
    if os.path.exists(CONFIG_FILE_NAME):
        try:
            ...
            current_parameters.update(loaded_params)
            ...
            if isinstance(loaded_params.get("score_weights"), dict):
                 ...
                 current_parameters["score_weights"] = DEFAULT_KPI_PARAMETERS["score_weights"].copy()
                 if "score_weights" in loaded_params : 
...
def get_database_connection() -> sqlite3.Connection:
    ...
    try:
        ...
        _initialize_custom_player_names_table(db_connection) 
        return db_connection
    except sqlite3.Error as error:
        console.print(Panel(f"[bold red]SQLite connection error:[/bold red]\n{error}", title="[bold red]DB Connection Error[/bold red]", border_style="white")) # MODIFIED
...
def _get_all_heroes_id_to_name_map_from_db(db_connection: sqlite3.Connection) -> Dict[int, str]:
    ...
    try:
        ...
        console.print(Panel(f"[bold red]SQLite error loading hero data:[/bold red]\n{error}\nHero names might not display correctly.", title="[bold red]DB Error[/bold red]", border_style="white")) # MODIFIED
        return {} 
...
def _visualize_json_structure_with_gravis(json_data: Dict[str, Any], item_identifier: Any, item_type_name: str = "Match") -> bool:
    ...
    if not VISUALIZATION_LIBRARIES_AVAILABLE:
        console.print(Panel("[bold yellow]Visualization libraries (gravis, networkx) are not installed.[/bold yellow]\nTo install: `pip install gravis networkx`\nSkipping visualization.", title="[yellow]Missing Libraries[/yellow]", border_style="white")) # MODIFIED
        return False
    if not json_data or not isinstance(json_data, (dict, list)): 
        console.print(Panel("[yellow]Invalid or empty JSON data provided for visualization.[/yellow]", title="[yellow]Visualization Warning[/yellow]", border_style="white")) # MODIFIED
        return False
    ...
    if not graph.nodes() or (len(graph.nodes()) == 1 and root_node_label in graph.nodes()):
         console.print(Panel("[yellow]Warning: The graph is empty or contains only the root node. Visualization might not be useful.[/yellow]", title="[yellow]Visualization Warning[/yellow]", border_style="white")) # MODIFIED
    ...
    try:
        ...
        try:
            ...
            console.print(Panel(f"[bold red]Error during basic Gravis (fallback) attempt:[/bold red]\n{error_simple_gravis}", title="[bold red]Visualization Error[/bold red]", border_style="white")) # MODIFIED
            return False 
    except Exception as error_gravis: 
        console.print(Panel(f"[bold red]General error during Gravis visualization:[/bold red]\n{error_gravis}", title="[bold red]Visualization Error[/bold red]", border_style="white")) # MODIFIED
        return False
...
def _identify_laning_players(players_data: List[Dict[str, Any]], hero_id_to_name_map: Dict[int, str]) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    ...
    for player_info in players_data:
        ...
        if is_radiant_player is None: 
            is_radiant_player = player_info.get("player_slot", 100) < 100 
...
def _get_player_kpis_at_specific_minute(player_match_data: Dict[str, Any], minute_mark: int, hero_id_to_name_map: Dict[int, str]) -> Dict[str, Any]:
    ...
    if level_timeseries and isinstance(level_timeseries, list) and minute_mark < len(level_timeseries): 
        ...
        xp_thresholds.extend([xp_thresholds[-1] + 600 * i for i in range(1, 16)]) 
...
def _extract_draft_order_string(match_data: Dict[str, Any], hero_id_to_name_map: Dict[int, str]) -> str:
    ...
    for draft_entry in picks_bans_data: 
        ...
        team_string = "Radiant" if draft_entry.get("team") == 0 else "Dire" 
...
def _perform_core_lane_analysis_for_match(match_id: int, db_connection: sqlite3.Connection, hero_id_to_name_map: Dict[int, str], player_custom_names_map: Dict[int, str]) -> Optional[Dict[str, Any]]:
    ...
    objectives_match_data = match_json_data.get("objectives") 
...
def set_player_name(
    ...
    console.print(Panel(f"[bold sky_blue1]Setting Custom Name for Account ID {account_id}[/bold sky_blue1]", expand=False, border_style="white")) # MODIFIED
    ...
    try:
        ...
        console.print(Panel(f"[bold red]Error: The custom name '{custom_name}' might already be assigned to another Account ID, or another integrity constraint failed.[/bold red]", title="[bold red]Integrity Error[/bold red]", border_style="white")) # MODIFIED
    except sqlite3.Error as error:
...
def delete_player_name(identifier_to_delete: str = typer.Argument(..., help="The Account ID or Custom Name to delete.")):
    ...
    console.print(Panel(f"[bold sky_blue1]Deleting Custom Name for Identifier '{identifier_to_delete}'[/bold sky_blue1]", expand=False, border_style="white")) # MODIFIED
    db_connection = get_database_connection()
...
def list_custom_names():
    ...
    console.print(Panel(panel_title, expand=False, border_style="white")) # MODIFIED
    db_connection = get_database_connection()
    try: 
    ...
    table = Table(title="Stored Custom Player Names", show_header=True, header_style="bold magenta", border_style="white", show_lines=True) # MODIFIED
...
def list_heroes():
    ...
    try:
        ...
        console.print(Panel(f"[bold red]SQLite query error for heroes:[/bold red]\n{error}\n[dim]Ensure 'heroes' table exists and is populated.[/dim]", title="[bold red]DB Error[/bold red]", border_style="white")) # MODIFIED
        db_connection.close()
        raise typer.Exit(code=1)
    finally:
        db_connection.close()
    ...
    table = Table(title="Stored Heroes (from Database)", show_header=True, header_style="bold magenta", border_style="white", show_lines=True) # MODIFIED
...
@app.command(name="list-matches", help="Lists stored matches. Filters: --limit (-l), --team (-t), --league (-L by ID or Name).")
def list_matches(
    ...
    league_identifier: Optional[str] = typer.Option(None, "--league", "-L", help="Filter matches by League ID or a part of the League Name (case-insensitive).") # Modified parameter
):
    panel_title_parts = ["[bold sky_blue1]:trophy: Listing Stored Matches"]
    if search_team_name:
        panel_title_parts.append(f" for Team '{search_team_name}'")
    if league_identifier is not None:
        panel_title_parts.append(f"{' and' if search_team_name else ' for'} League '{league_identifier}'")
    panel_title_parts.append(" :trophy:[/bold sky_blue1]")
    
    console.print(Panel(Text.from_markup("".join(panel_title_parts)), expand=False, border_style="white"))
    ...
    # Attempt to parse league_identifier as an ID for potential SQL pre-filtering
    league_id_as_int: Optional[int] = None
    is_league_filter_numeric_id = False
    if league_identifier is not None:
        try:
            league_id_as_int = int(league_identifier)
            is_league_filter_numeric_id = True
        except ValueError:
            is_league_filter_numeric_id = False # It's a name string

    if search_team_name:
        team_name_like_pattern = f'%"{search_team_name}"%' 
        team_clause = "(data LIKE ? OR data LIKE ? OR data LIKE ? OR data LIKE ?)"
        where_clauses.append(team_clause)
        query_parameters_list.extend([team_name_like_pattern] * 4)

    if league_identifier is not None:
        if is_league_filter_numeric_id and league_id_as_int is not None:
            # Broad SQL filter for league ID
            league_id_like_pattern = f'%"leagueid":%{league_id_as_int}%'
            league_clause = "data LIKE ?"
            where_clauses.append(league_clause)
            query_parameters_list.append(league_id_like_pattern)
        else: # It's a name string, try to broadly filter by name in JSON
            # This SQL LIKE for nested name is very approximate and might fetch too many rows
            # or miss some, depending on JSON structure variations.
            # Python filtering will be the definitive check.
            league_name_like_pattern = f'%"league":%name%:%{league_identifier}%'
            league_clause = "data LIKE ?"
            where_clauses.append(league_clause)
            query_parameters_list.append(league_name_like_pattern)


    if where_clauses:
        query_string += "WHERE " + " AND ".join(where_clauses)
    
    query_string += " ORDER BY fetched_at DESC" 
    
    try:
        all_potentially_relevant_rows = db_connection.execute(query_string, tuple(query_parameters_list)).fetchall()
    except sqlite3.Error as error:
        console.print(Panel(f"[bold red]SQLite query error for matches:[/bold red]\n{error}", title="[bold red]DB Error[/bold red]", border_style="white"))
        db_connection.close()
        raise typer.Exit(code=1)

    if not all_potentially_relevant_rows:
        message = "[yellow]No matches found in the database"
        if search_team_name or league_identifier is not None:
            message += " matching the initial SQL filter criteria."
        else: message += "."
        console.print(Padding(message, (1,2)))
        db_connection.close()
        return

    # --- Python-based filtering for accuracy ---
    filtered_match_rows_data = []
    for match_row_item in all_potentially_relevant_rows:
        try:
            match_json_data: Dict[str, Any] = json.loads(match_row_item["data"])
        except json.JSONDecodeError:
            continue

        passes_team_filter = True
        if search_team_name:
            passes_team_filter = False 
            ...
            if team_name_lower in radiant_team_name.lower() or \
        ...
        passes_league_filter = True
        if league_identifier is not None:
            passes_league_filter = False # Must explicitly pass
            if is_league_filter_numeric_id and league_id_as_int is not None:
                # Exact match for League ID
                if match_json_data.get("leagueid") == league_id_as_int:
                    passes_league_filter = True
            else:
                # Case-insensitive substring match for League Name
                league_info = match_json_data.get("league")
                if isinstance(league_info, dict):
                    current_league_name = str(league_info.get("name", "")).lower()
                    if league_identifier.lower() in current_league_name:
                        passes_league_filter = True
        
        if passes_team_filter and passes_league_filter:
            filtered_match_rows_data.append(match_row_item)

    if limit_matches is not None and limit_matches > 0:
        final_match_rows_to_display = filtered_match_rows_data[:limit_matches]
    else:
        final_match_rows_to_display = filtered_match_rows_data

    if not final_match_rows_to_display:
        message = "[yellow]No matches found after applying detailed filters."
        if search_team_name: message += f" (Team: '{search_team_name}')"
        if league_identifier is not None: message += f" (League: '{league_identifier}')"
        console.print(Padding(message, (1,2)))
        db_connection.close()
        return

    table = Table(title="Stored Matches with Player Details", show_header=True, header_style="bold magenta", border_style="white", show_lines=True, expand=True)
    table.add_column("Match ID", style="dim blue", width=12, justify="center")
    table.add_column("League (ID/Name)", style="dim", width=25, justify="left", overflow="fold") # Updated column
    ...
    for match_row_item in final_match_rows_to_display:
        match_id = match_row_item["match_id"]
        fetched_at_timestamp = match_row_item["fetched_at"]
        match_json_data: Dict[str, Any] = json.loads(match_row_item["data"])

        league_id_val = match_json_data.get('leagueid')
        league_name_val = match_json_data.get('league', {}).get('name', 'N/A')
        league_display = f"{league_name_val} (ID: {league_id_val})" if league_id_val is not None else league_name_val
        ...
        for player_info_item in players_in_match_data:
            ...
            # Use _get_hero_display_name_from_hero_id if it's defined, otherwise adapt
            ...
            player_identifier_string = player_custom_names_map.get(account_id_value, str(account_id_value) if account_id_value is not None else player_info_item.get("personaname", "N/A"))
            ...
            if is_radiant_player_flag is None: 
                player_slot_value = player_info_item.get("player_slot")
                if player_slot_value is not None: is_radiant_player_flag = player_slot_value < 100
                else: is_radiant_player_flag = False 

            if is_radiant_player_flag: radiant_players_details_list.append(player_detail_string)
            else: dire_players_details_list.append(player_detail_string)
        ...
        table.add_row(
            ...
            radiant_team_name, 
            dire_team_name, 
            f"{radiant_score}-{dire_score_val}", 
            winner_team_name, 
            radiant_players_display_string, 
            dire_players_display_string, 
    ...
    if matches_displayed_count > 0:
        ...
        filter_messages = []
        if search_team_name: filter_messages.append(f"team: '{search_team_name}'")
        if league_identifier is not None: filter_messages.append(f"league: '{league_identifier}'")
        if filter_messages:
            console.print(f"[dim]Filters active: {'; '.join(filter_messages)}[/dim]")
    elif search_team_name or league_identifier is not None:
        message = "[yellow]No matches found matching all specified filter criteria after detailed check."
        if search_team_name: message += f" (Team: '{search_team_name}')"
        if league_identifier is not None: message += f" (League: '{league_identifier}')"
        console.print(Padding(message, (1,2)))


@app.command(name="list-all-teams", help="Lists all unique team names and tags found in the database.")
def list_all_teams():
    """Scans all matches and lists unique team names and tags."""
    panel_title = Text.from_markup("[bold sky_blue1]:shield: Listing All Unique Teams :shield:[/bold sky_blue1]")
    console.print(Panel(panel_title, expand=False, border_style="white"))
    db_connection = get_database_connection()
    
    all_teams = set()
    try:
        match_rows = db_connection.execute("SELECT data FROM matches").fetchall()
    except sqlite3.Error as e:
        console.print(Panel(f"[bold red]SQLite error fetching matches for team listing:[/bold red]\n{e}", title="[bold red]DB Error[/bold red]", border_style="white"))
        db_connection.close()
        raise typer.Exit(code=1)
    
    if not match_rows:
        console.print(Padding("[yellow]No matches found in the database to extract team names from.[/yellow]", (1,2)))
        db_connection.close()
        return
    ...
    for row in match_rows:
        try:
            ...
            # Check top-level radiant_name and dire_name
            if match_data.get("radiant_name"):
                all_teams.add(str(match_data["radiant_name"]).strip())
            if match_data.get("dire_name"):
                all_teams.add(str(match_data["dire_name"]).strip())

            # Check radiant_team and dire_team objects
            radiant_team_obj = match_data.get("radiant_team")
            if isinstance(radiant_team_obj, dict):
                if radiant_team_obj.get("name"):
                    all_teams.add(str(radiant_team_obj["name"]).strip())
                if radiant_team_obj.get("tag"):
                    all_teams.add(str(radiant_team_obj["tag"]).strip())
            
            dire_team_obj = match_data.get("dire_team")
            if isinstance(dire_team_obj, dict):
                if dire_team_obj.get("name"):
                    all_teams.add(str(dire_team_obj["name"]).strip())
                if dire_team_obj.get("tag"):
                    all_teams.add(str(dire_team_obj["tag"]).strip())

        except json.JSONDecodeError:
            # console.print(f"[yellow]Warning: Could not parse JSON for a match while listing teams. Skipping.[/yellow]")
            continue # Skip problematic rows
    db_connection.close()

    if not all_teams:
        console.print(Padding("[yellow]No team names or tags found in the match data.[/yellow]", (1,2)))
        return

    table = Table(title="Unique Team Names & Tags", show_header=True, header_style="bold magenta", border_style="white", show_lines=True)
    table.add_column("Team Name/Tag", style="bright_white", min_width=30)
    
    sorted_teams = sorted(list(all_teams), key=lambda x: x.lower())
    for team_name in sorted_teams:
        if team_name: # Ensure not adding empty strings if any slip through
             table.add_row(team_name)
    
    console.print(table)
    console.print(f"\n[dim]Found {len(sorted_teams)} unique team names/tags.[/dim]")


@app.command(name="list-all-leagues", help="Lists all unique leagues found in the database.")
def list_all_leagues():
    """Scans all matches and lists unique league names and their IDs."""
    panel_title = Text.from_markup("[bold sky_blue1]:trophy: Listing All Unique Leagues :trophy:[/bold sky_blue1]")
    console.print(Panel(panel_title, expand=False, border_style="white"))
    db_connection = get_database_connection()
    
    all_leagues: Dict[int, str] = {} # Store as {league_id: league_name}
    
    try:
        match_rows = db_connection.execute("SELECT data FROM matches").fetchall()
    except sqlite3.Error as e:
        console.print(Panel(f"[bold red]SQLite error fetching matches for league listing:[/bold red]\n{e}", title="[bold red]DB Error[/bold red]", border_style="white"))
        db_connection.close()
        raise typer.Exit(code=1)

    if not match_rows:
        console.print(Padding("[yellow]No matches found in the database to extract league information from.[/yellow]", (1,2)))
        db_connection.close()
        return
    ...
    for row in match_rows:
        try:
            ...
            league_info = match_data.get("league") # This is a dict containing 'name', 'tier', etc.
            
            if league_id is not None:
                league_name = "Unknown League Name" # Default if name not found
                if isinstance(league_info, dict) and league_info.get("name"):
                    league_name = str(league_info["name"]).strip()
                
                # Add or update league name if a better one is found (e.g. not "Unknown")
                if league_id not in all_leagues or all_leagues[league_id] == "Unknown League Name":
                    all_leagues[league_id] = league_name
                    
        except json.JSONDecodeError:
            # console.print(f"[yellow]Warning: Could not parse JSON for a match while listing leagues. Skipping.[/yellow]")
            continue # Skip problematic rows
    db_connection.close()

    if not all_leagues:
        console.print(Padding("[yellow]No league information found in the match data.[/yellow]", (1,2)))
        return

    table = Table(title="Unique Leagues", show_header=True, header_style="bold magenta", border_style="white", show_lines=True)
    table.add_column("League ID", style="dim cyan", width=12, justify="center")
    table.add_column("League Name", style="bright_white", min_width=30)
    
    # Sort by League ID for consistent output
    sorted_league_ids = sorted(all_leagues.keys())
    
    for league_id in sorted_league_ids:
        table.add_row(str(league_id), all_leagues[league_id])
    
    console.print(table)
    console.print(f"\n[dim]Found {len(all_leagues)} unique leagues.[/dim]")
...
def find_player_hero_matches(
    ...
    if hero_name_input_argument:
        ...
        if target_hero_id_value is None:
            console.print(Panel(f"[bold red]Hero '{hero_name_input_argument}' not found or ambiguous.[/bold red]\nTry 'list-heroes' to see available internal names.", title="[bold red]Hero Not Found[/bold red]", border_style="white")) # MODIFIED
            db_connection.close()
            raise typer.Exit(code=1)
    ...
    console.print(Panel(panel_title_text, expand=False, border_style="white")) # MODIFIED

    search_target_account_id_value: Optional[int] = None
    search_by_personaname_fallback_flag = False
    display_search_term_string = player_identifier_argument 
    try:
    ...
    query_parameters = (f'%{sql_search_term_string}%',) 
    ...
    table = Table(title=table_title_string, show_header=True, header_style="bold magenta", border_style="white", show_lines=True) # MODIFIED
    table.add_column("Match ID", style="dim purple", width=12, justify="center")
    table.add_column("Player's Hero", style="bold cyan", min_width=20) 
    table.add_column("Radiant Team", style="bold bright_cyan", min_width=20)
    table.add_column("Dire Team", style="bold bright_red", min_width=20)
    table.add_column("Score (R-D)", justify="center", width=12, style="white")
    table.add_column("Winner", min_width=20, justify="center")
...
def show_match_json(
    ...
    console.print(Panel(f"[bold sky_blue1]JSON Data for Match ID {match_id_argument}[/bold sky_blue1]", expand=False, border_style="white")) # MODIFIED
    db_connection = get_database_connection()
    ...
    try:
        match_json_parsed_data = json.loads(json_string_data_from_db) 
        ...
        if output_file_path: 
            try:
                ...
                with open(output_file_absolute_path, 'w') as file_writer:
                    json.dump(match_json_parsed_data, file_writer, indent=2) 
        ...
        if should_print_to_console and match_json_parsed_data:
            console.print(Panel(Syntax(json.dumps(match_json_parsed_data, indent=2), "json", theme="paraiso-dark", line_numbers=True, word_wrap=True), title=f"JSON for Match ID {match_id_argument}", border_style="white", expand=False)) # MODIFIED
    
    except json.JSONDecodeError: 
        console.print(Panel(f"[bold red]Error: Stored data for match ID {match_id_argument} is not valid JSON.[/bold red]", title="[bold red]JSON Error[/bold red]", border_style="white")) # MODIFIED
        if output_file_path: 
            try:
                output_file_absolute_path = os.path.abspath(output_file_path)
                output_directory = os.path.dirname(output_file_absolute_path)
...
def database_statistics():
    ...
    table = Table(title="Database Overview", show_lines=True, header_style="bold magenta", border_style="white") # MODIFIED
...
def player_summary(player_identifier_argument: str = typer.Argument(..., help="Player's Account ID, Custom Name, or current Persona Name.")):
    ...
    search_by_personaname_fallback_flag = False
    display_player_term_string = player_identifier_argument 
    sql_like_prefilter_term_string: str 
    ...
    try:
        ...
        if not resolved_by_custom_name_flag:
            ...
            sql_like_prefilter_term_string = f'%"personaname": "%{escaped_player_identifier}%"%' 
            console.print(f"[dim]Searching by Persona Name containing: '{player_identifier_argument}'[/dim]")
    ...
    for match_row_item in match_rows_data:
        ...
        for player_info_item_loop in match_json_data.get("players", []):
            ...
            if target_account_id_value is not None: 
                if current_account_id_value == target_account_id_value: player_matches_criteria_flag = True
            elif search_by_personaname_fallback_flag: 
            ...
            if player_matches_criteria_flag: player_in_this_match_info = player_info_item_loop; break 
        ...
        if player_in_this_match_info:
            ...
            if player_is_radiant_value is None and "player_slot" in player_in_this_match_info: 
    ...
    console.print(Panel(summary_text, title="[b]Overall Player Performance[/b]", border_style="white", expand=False)) # MODIFIED
    ...
    if hero_performance_data:
        ...
        top_heroes_table = Table(show_header=True, header_style="bold cyan", border_style="white", show_lines=True) # MODIFIED
        top_heroes_table.add_column("Hero", style="bold cyan") 
        ...
        for hero_id_value, stats_dictionary in sorted_hero_performance_list[:5]: 
...
def hero_summary(hero_identifier_argument: str = typer.Argument(..., help="The name (e.g. 'antimage') or ID of the hero.")):
    ...
    try:
        ...
        if target_hero_id_value is None:
            console.print(Panel(f"[bold red]Hero name '{hero_identifier_argument}' not found or ambiguous. Try 'list-heroes'.[/bold red]", title="[bold red]Hero Not Found[/bold red]", border_style="white")) # MODIFIED
            if db_connection: db_connection.close()
            return
    ...
    for match_row_item in match_rows_data:
        ...
        for player_info_item_loop in match_json_data.get("players", []):
            if player_info_item_loop.get("hero_id") == target_hero_id_value:
                player_who_picked_hero_info = player_info_item_loop; break 
        ...
        if player_who_picked_hero_info:
            ...
            if picker_is_radiant_value is None and "player_slot" in player_who_picked_hero_info: 
            ...
            if radiant_won_match_flag is not None and picker_is_radiant_value is not None:
                if (radiant_won_match_flag is True and picker_is_radiant_value) or \
                   (radiant_won_match_flag is False and not picker_is_radiant_value):
                    total_hero_wins_count += 1; picker_won_this_match_flag = True
    ...
    console.print(Panel(hero_summary_text, title="[b]Overall Hero Performance[/b]", border_style="white", expand=False)) # MODIFIED
    ...
    if player_performance_with_hero_data:
        ...
        top_players_table = Table(show_header=True, header_style="bold cyan", border_style="white", show_lines=True) # MODIFIED
        ...
        for account_id_value, stats_dictionary in sorted_player_performance_list[:5]: 
            player_display_name_string = player_custom_names_map.get(account_id_value, f"ID: {account_id_value}")
...
def analyze_lanes_command(match_id_argument: int = typer.Argument(..., help="The Match ID to analyze.")):
    ...
    if not analysis_data or analysis_data.get("error"):
        console.print(Panel(f"[bold red]Could not analyze lanes for match {match_id_argument}:[/bold red]\n{analysis_data.get('error', 'Unknown error')}", title="[bold red]Analysis Error[/bold red]", border_style="white")) # MODIFIED
        if db_connection: db_connection.close()
        return
    ...
    if not match_json_data_for_tables:
        console.print(Panel(f"[bold red]Could not re-fetch match data for console display for match {match_id_argument}. Detailed player tables might be incomplete.[/bold red]", title="[bold red]Data Error[/bold red]", border_style="white")) # MODIFIED
        players_data_for_tables_display = [] 
        lanes_assignments_for_tables_display = {"top":{}, "mid":{}, "bot":{}} 
    else:
    ...
    for lane_name_key, lane_information in analysis_data.get("lanes", {}).items():
        if lane_name_key == "unknown": continue 

        console.print(Panel(f"Lane Analysis: [b]{lane_name_key.upper()}[/b]", style="bold yellow", expand=False, border_style="white")) # MODIFIED
        ...
        for team_name_string, players_in_lane_for_team_display in teams_in_lane_for_table_display.items():
            ...
            table = Table(
                ...
                border_style="white", show_lines=True # MODIFIED
            )
            table.add_column("Player", style="white", min_width=15, overflow="fold")
            table.add_column("Hero", style="bold cyan", min_width=15) 
        ...
        console.print(Panel(lane_summary_panel_content, title=f"Overall {lane_name_key.upper()} Lane Summary (@{primary_minute_mark}min)", expand=False, border_style="white")) # MODIFIED
        console.print("-" * console.width) 


@app.command(
    name="export-match-ids",
    help="Exports match IDs to CSV. Filters: --output-file (-o) is required. Optional: --team (-t), --league (-L by ID or Name)."
)
def export_match_ids(
    output_file_path_arg: str = typer.Option(
        ...,  # Ellipsis makes it a required option
        "--output-file", "-o",
        help="Path to save the CSV file (e.g., all_match_ids.csv).",
        dir_okay=False,
        writable=True,
        show_default=False # No default for a required option path
    ),
    team_name_filter: Optional[str] = typer.Option(
        None,
        "--team", "-t",
        help="Filter by a team name or tag (case-insensitive). Exports matches where this team participated."
    ),
    league_identifier: Optional[str] = typer.Option(
        None,
        "--league", "-L",  # Using -L for league to avoid conflict with -l (limit)
        help="Filter by League ID or a part of the League Name (case-insensitive)."
    )
):
    """
    Exports match IDs from the database to a CSV file.
    The output can be filtered by team name/tag and by league (ID or name).
    The --output-file (-o) option is required.
    """
    title_suffix_parts = []
    if team_name_filter:
        title_suffix_parts.append(f"for Team '{team_name_filter}'")
    if league_identifier:
        title_suffix_parts.append(f"{' and ' if title_suffix_parts else 'for '}League '{league_identifier}'")
    
    title_suffix = "".join(title_suffix_parts)
    console.print(Panel(f"[bold sky_blue1]Exporting Match IDs{title_suffix} to '{output_file_path_arg}'[/bold sky_blue1]", expand=False, border_style="white")) # Assuming white borders
    
    db_connection = get_database_connection()
    match_ids_to_export = []
    
    query_string = "SELECT match_id, data FROM matches " # Fetch data for Python-side filtering
    query_parameters_list: list = []
    where_clauses: List[str] = []
    ...
    if team_name_filter:
        team_name_like_pattern = f'%"{team_name_filter}"%' # Basic JSON string search
        team_clause = "(data LIKE ? OR data LIKE ? OR data LIKE ? OR data LIKE ?)" # Search in various team name fields
        where_clauses.append(team_clause)
        query_parameters_list.extend([team_name_like_pattern] * 4)

    if league_identifier is not None:
        if is_league_filter_numeric_id and league_id_as_int is not None:
            league_id_like_pattern = f'%"leagueid":%{league_id_as_int}%' # Broad search for "leagueid":123
            league_clause = "data LIKE ?"
            where_clauses.append(league_clause)
            query_parameters_list.append(league_id_like_pattern)
        else: # It's a name string, try to broadly filter by name in JSON
            # This SQL LIKE for nested name is approximate. Python filtering is key.
            league_name_like_pattern = f'%"league":%name%:%{league_identifier}%'
            league_clause = "data LIKE ?"
            where_clauses.append(league_clause)
            query_parameters_list.append(league_name_like_pattern)
    ...
    query_string += " ORDER BY match_id ASC" # Consistent ordering
    
    try:
        all_potentially_relevant_rows = db_connection.execute(query_string, tuple(query_parameters_list)).fetchall()
    except sqlite3.Error as error:
        console.print(Panel(f"[bold red]SQLite error fetching match data for export:[/bold red]\n{error}", title="[bold red]DB Error[/bold red]", border_style="white"))
        db_connection.close()
        raise typer.Exit(code=1)
    
    if not all_potentially_relevant_rows:
        message = "[yellow]No matches found in the database"
        if team_name_filter or league_identifier:
            message += " matching the initial SQL filter criteria."
        else: message += "."
        console.print(Padding(message, (1,2)))
        db_connection.close()
    ...
    # --- Python-based filtering for accuracy ---
    for row in all_potentially_relevant_rows:
        match_id = row["match_id"]
        match_data_str = row["data"]
        
        try:
            match_json = json.loads(match_data_str)
        except json.JSONDecodeError:
            # console.print(f"[yellow]Warning: Skipping match ID {match_id} due to JSON decode error during export filtering.[/yellow]")
            continue

        passes_team_filter = True
        if team_name_filter:
            passes_team_filter = False 
            team_name_lower = team_name_filter.lower()
            if team_name_lower in str(match_json.get("radiant_name", "")).lower() or \
               team_name_lower in str(match_json.get("dire_name", "")).lower():
                passes_team_filter = True
            if not passes_team_filter:
                radiant_team_obj = match_json.get("radiant_team")
                if isinstance(radiant_team_obj, dict) and (team_name_lower in str(radiant_team_obj.get("name", "")).lower() or team_name_lower in str(radiant_team_obj.get("tag", "")).lower()):
                    passes_team_filter = True
            if not passes_team_filter:
                dire_team_obj = match_json.get("dire_team")
                if isinstance(dire_team_obj, dict) and (team_name_lower in str(dire_team_obj.get("name", "")).lower() or team_name_lower in str(dire_team_obj.get("tag", "")).lower()):
                    passes_team_filter = True
        ...
        if league_identifier is not None:
            ...
            if is_league_filter_numeric_id and league_id_as_int is not None:
                # Exact match for League ID
                if match_json.get("leagueid") == league_id_as_int:
                    passes_league_filter = True
            else:
                # Case-insensitive substring match for League Name
                league_info = match_json.get("league")
                if isinstance(league_info, dict):
                    current_league_name = str(league_info.get("name", "")).lower()
                    if league_identifier.lower() in current_league_name:
                        passes_league_filter = True
        ...
        if passes_team_filter and passes_league_filter:
            match_ids_to_export.append(match_id)
            
    db_connection.close()

    if not match_ids_to_export:
        message_parts = ["[yellow]No match IDs found matching all specified filter criteria after detailed check.[/yellow]"]
        if team_name_filter: message_parts.append(f" (Team: '{team_name_filter}')")
        if league_identifier: message_parts.append(f" (League: '{league_identifier}')")
        console.print(Padding("".join(message_parts), (1,2)))
        return
    ...
    try:
        ...
        with open(output_file_absolute_path, 'w', newline='') as csv_file_writer:
            ...
            csv_writer_object.writerow(["match_id"]) # Header
            for match_id_item in match_ids_to_export:
                csv_writer_object.writerow([match_id_item])
        console.print(f"[green]Successfully exported {len(match_ids_to_export)} match IDs to: {output_file_absolute_path}[/green]")
    except Exception as file_system_error:
        console.print(Panel(f"[bold red]An unexpected error occurred while writing to file '{output_file_path_arg}':[/bold red]\n{file_system_error}", title="[bold red]File Save Error[/bold red]", border_style="white"))
...
def batch_analyze_lanes(
    ...
    console.print(Panel(f"[bold sky_blue1]Batch Lane Analysis from '{input_csv_file_path}' to '{output_csv_file_path}'[/bold sky_blue1]", expand=False, border_style="white")) # MODIFIED
    ...
    try: 
        with open(input_csv_file_path, 'r', newline='') as csv_file_reader:
            ...
            if "match_id" not in csv_dict_reader.fieldnames: 
                console.print(Panel(f"[bold red]Error: Input CSV '{input_csv_file_path}' must contain a 'match_id' column header.[/bold red]", title="[bold red]CSV Error[/bold red]", border_style="white")) # MODIFIED
                raise typer.Exit(code=1)
    ...
    if output_directory and not os.path.exists(output_directory): 
        try:
            ...
            console.print(Panel(f"[bold red]Error creating output directory '{output_directory}':[/bold red]\n{os_error}", title="[bold red]Directory Error[/bold red]", border_style="white")) # MODIFIED
            db_connection.close()
            raise typer.Exit(code=1)
            
    csv_field_names_list = [ 
    ...
    with Progress(
        ...
        TimeRemainingColumn(), TimeElapsedColumn(), console=console, transient=True 
    ) as progress_bar:
...
def manage_kpi_parameters_config(
    ...
    global KPI_PARAMETERS 
    current_loaded_parameters = _load_kpi_parameters() 
    parameters_were_changed = False

    if reset_config_to_defaults:
        current_loaded_parameters = DEFAULT_KPI_PARAMETERS.copy() 
        _save_kpi_parameters(current_loaded_parameters) 
        KPI_PARAMETERS = current_loaded_parameters.copy() 
        console.print(Panel(f"[green]KPI parameters have been reset to defaults and saved to {CONFIG_FILE_NAME}.[/green]", title="[green]Config Reset[/green]", border_style="white")) # MODIFIED
        if not show_current_config: return 
    ...
    if parameters_were_changed and not reset_config_to_defaults: 
        _save_kpi_parameters(current_loaded_parameters)
        KPI_PARAMETERS = current_loaded_parameters.copy() 
        console.print(Panel(f"[green]KPI parameters updated and saved to {CONFIG_FILE_NAME}.[/green]", title="[green]Config Updated[/green]", border_style="white")) # MODIFIED

    if show_current_config: 
        console.print(Panel(f"Current Lane Analysis KPI Parameters (from [underline blue]{CONFIG_FILE_NAME}[/underline blue])", expand=False, border_style="white", title_align="left", padding=(1,2))) # MODIFIED
...
if __name__ == "__main__":
    console.print(Panel(Text.from_markup("[bold green]:computer: OpenDota DB Viewer & Analyzer CLI :chart_increasing:[/bold green]"), expand=False, border_style="white")) # MODIFIED
    app()
