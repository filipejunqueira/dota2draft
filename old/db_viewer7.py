...
def list_matches(
    ...
    if search_team_name:
        ...
        like_pattern = f'%"{search_team_name}"%' 
        query_parameters_list.extend([like_pattern] * 4) 
    ...
    table = Table(title="Stored Matches with Player Details", show_header=True, header_style="bold magenta", border_style="white", show_lines=True, expand=True) # MODIFIED
        ...
        for player_info_item in players_in_match_data:
            ...
            if is_radiant_player_flag is None: 
                ...
                if player_slot_value is not None:
                    is_radiant_player_flag = player_slot_value < 100 
                else:
                    is_radiant_player_flag = False 
...
@app.command(name="export-match-ids", help="Exports match IDs from the database to a CSV file. Can be filtered by team or league.")
def export_match_ids(
    output_file_path_arg: str = typer.Option(..., "--output-file", "-o", help="Path to save the CSV file (e.g., all_match_ids.csv).", dir_okay=False, writable=True),
    team_name_filter: Optional[str] = typer.Option(None, "--team", help="Filter by a team name or tag (case-insensitive). Exports matches where this team participated."),
    league_id_filter: Optional[int] = typer.Option(None, "--league", help="Filter by a specific League ID. Exports matches from this league only.")
):
    """Exports match_ids from the 'matches' table to a CSV file, with optional team and league filters."""
    filter_active = team_name_filter or (league_id_filter is not None)
    title_suffix = ""
    if team_name_filter:
        title_suffix += f" for Team '{team_name_filter}'"
    if league_id_filter is not None:
        title_suffix += f"{' and' if team_name_filter else ''} for League ID {league_id_filter}"

    console.print(Panel(f"[bold sky_blue1]Exporting Match IDs{title_suffix} to '{output_file_path_arg}'[/bold sky_blue1]", expand=False, border_style="white"))
    
    db_connection = get_database_connection()
    match_ids_to_export = []
    
    # Base query
    query = "SELECT match_id, data FROM matches ORDER BY match_id ASC"
    
    try:
        all_match_rows = db_connection.execute(query).fetchall()
    except sqlite3.Error as error:
    ...
    if not all_match_rows:
        console.print(Padding("[yellow]No matches found in the database to export.[/yellow]",(1,2)))
        db_connection.close()
        return

    # Filter in Python after fetching, as JSON parsing is needed
    for row in all_match_rows:
        ...
        if team_name_filter:
            passes_team_filter = False # Assume false until a match is found
            ...
            # Check team objects if not already passed
            if not passes_team_filter:
                radiant_team_obj = match_json.get("radiant_team")
                if isinstance(radiant_team_obj, dict):
                    if team_name_lower in str(radiant_team_obj.get("name", "")).lower() or \
                       team_name_lower in str(radiant_team_obj.get("tag", "")).lower():
                        passes_team_filter = True
            
            if not passes_team_filter:
                dire_team_obj = match_json.get("dire_team")
                if isinstance(dire_team_obj, dict):
                    if team_name_lower in str(dire_team_obj.get("name", "")).lower() or \
                       team_name_lower in str(dire_team_obj.get("tag", "")).lower():
                        passes_team_filter = True
        
        passes_league_filter = True
        if league_id_filter is not None:
            if match_json.get("leagueid") != league_id_filter:
                passes_league_filter = False
    ...
    if not match_ids_to_export:
        message = "[yellow]No match IDs found matching the specified criteria.[/yellow]"
        if team_name_filter: message += f" (Team: '{team_name_filter}')"
        if league_id_filter is not None: message += f" (League ID: {league_id_filter})"
        console.print(Padding(message, (1,2)))
        return
