    ...
    if os.path.exists(CONFIG_FILE_NAME):
        try:
            ...
            if isinstance(loaded_params.get("score_weights"), dict):
                 ...
                 current_parameters["score_weights"] = DEFAULT_KPI_PARAMETERS["score_weights"].copy()
                 if "score_weights" in loaded_params : # If it existed but was invalid
...
def _get_all_heroes_id_to_name_map_from_db(db_connection: sqlite3.Connection) -> Dict[int, str]:
    ...
    try:
        ...
        for hero_row in cursor.fetchall(): hero_id_to_name_map[hero_row["hero_id"]] = hero_row["name"]
    except sqlite3.Error as error:
        ...
        return {}
    if not hero_id_to_name_map: console.print("[yellow]Warning: Hero map from database is empty. Hero names may not display correctly.[/yellow]")
...
def _get_all_player_custom_names_map_from_db(db_connection: sqlite3.Connection) -> Dict[int, str]:
    ...
    try:
        ...
        for name_row in cursor.fetchall(): custom_names_map[name_row["account_id"]] = name_row["custom_name"]
    except sqlite3.Error as error:
...
def _visualize_json_structure_with_gravis(json_data: Dict[str, Any], item_identifier: Any, item_type_name: str = "Match") -> bool:
    ...
    if gv: console.print(f"[dim]Using Gravis version: {gv.__version__}[/dim]")
    ...
    def add_nodes_and_edges_recursively(data_node: Any, parent_node_name: Optional[str] = None):
        ...
        if isinstance(data_node, dict):
            for key, value in data_node.items():
                ...
                if isinstance(value, (dict, list)): add_nodes_and_edges_recursively(value, current_node_name)
            ...
            for index, item in enumerate(data_node):
                ...
                if parent_node_name: graph.add_edge(parent_node_name, current_node_name)
                if isinstance(item, (dict, list)): add_nodes_and_edges_recursively(item, current_node_name)
    ...
    if not graph.nodes() or (len(graph.nodes()) == 1 and root_node_label in graph.nodes()):
         ...
         graph.nodes[root_node_label]['title'] = json.dumps(json_data, indent=2) # Add full JSON to root if it's the only node

    output_filename_absolute = os.path.abspath(f"{item_type_name.lower()}_{item_identifier}_visualization.html")
    try:
        figure = gv.d3(graph, graph_height=800, node_label_data_source='label', show_menu=True, zoom_factor=0.7, details_min_height=150, details_max_height=300, use_edge_size_normalization=True, edge_size_data_source='weight', use_node_size_normalization=True, node_size_data_source='size')
        ...
        if webbrowser:
            try: webbrowser.open(f"file://{output_filename_absolute}")
            except webbrowser.Error as wb_error: console.print(f"[yellow]Could not open visualization in browser: {wb_error}. Please open the file manually.[/yellow]")
        return True
    except TypeError as type_error:
        console.print(f"[bold red]Gravis TypeError (likely version incompatibility): {type_error}[/bold red]")
        ...
        try:
            ...
            console.print(f"[green]Successfully generated basic interactive visualization: {output_filename_absolute}[/green]")
            if webbrowser:
                try: webbrowser.open(f"file://{output_filename_absolute}")
                except webbrowser.Error as wb_error: console.print(f"[yellow]Could not open basic visualization in browser: {wb_error}.[/yellow]")
            return True
...
def _identify_laning_players(players_data: List[Dict[str, Any]], hero_id_to_name_map: Dict[int, str]) -> Dict[str, Dict[str, List[Dict[str, Any]]]]:
    ...
    for player_info in players_data:
        ...
        if is_radiant_player is None: is_radiant_player = player_info.get("player_slot", 100) < 100
...
def _get_player_kpis_at_specific_minute(player_match_data: Dict[str, Any], minute_mark: int, hero_id_to_name_map: Dict[int, str]) -> Dict[str, Any]:
    ...
    if level_timeseries and isinstance(level_timeseries, list) and minute_mark < len(level_timeseries): 
        ...
        xp_thresholds.extend([xp_thresholds[-1] + 600 * i for i in range(1, 16)]) # Levels 16-30
        level = 1
        current_player_xp = kpis["xp"]
        for index, threshold in enumerate(xp_thresholds):
            if current_player_xp >= threshold: level = index + 1
            else: break
...
def _analyze_lane_kill_death_events_for_player(laner_player_object: Dict[str, Any], all_players_data: List[Dict[str, Any]], opposing_laner_hero_names: List[str], time_limit_seconds: int) -> Tuple[int, int]:
    ...
    for potential_killer_object in all_players_data:
        if potential_killer_object.get("player_slot") == laner_player_object.get("player_slot"): continue # Self
        if potential_killer_object.get("isRadiant") == laner_player_object.get("isRadiant"): continue # Same team
...
def _extract_draft_order_string(match_data: Dict[str, Any], hero_id_to_name_map: Dict[int, str]) -> str:
    ...
    draft_entries = []
    for draft_entry in picks_bans_data: # Assuming OpenDota provides it sorted by "order"
...
def _perform_core_lane_analysis_for_match(match_id: int, db_connection: sqlite3.Connection, hero_id_to_name_map: Dict[int, str], player_custom_names_map: Dict[int, str]) -> Optional[Dict[str, Any]]:
    cursor = db_connection.cursor()
    try: cursor.execute("SELECT data FROM matches WHERE match_id = ?", (match_id,)); match_row_data = cursor.fetchone()
    except sqlite3.Error as error: return {"error": f"SQLite query error for match {match_id}: {error}"}
    if not match_row_data or not match_row_data["data"]: return {"error": f"No data found in DB for match ID {match_id}."}
    try: match_json_data = json.loads(match_row_data["data"])
    except json.JSONDecodeError: return {"error": f"Invalid JSON for match ID {match_id}."}
    
    players_match_data = match_json_data.get("players", [])
    if not players_match_data: return {"error": f"No player data in JSON for match ID {match_id}."}

    for player_index, player_info in enumerate(players_match_data):
        player_account_id = player_info.get("account_id")
        display_name = player_info.get("personaname", f"Slot {player_info.get('player_slot', player_index)}")
        if player_account_id and player_account_id in player_custom_names_map:
            display_name = player_custom_names_map[player_account_id]
    ...
    for lane_name_key in ["top", "mid", "bot"]:
        ...
        if not teams_in_this_lane or (not teams_in_this_lane["radiant"] and not teams_in_this_lane["dire"]):
            analysis_results["lanes"][lane_name_key] = {"radiant_score": 0, "dire_score": 0, "verdict_text": "No players in lane", "summary_details": {}}
            continue

        lane_summary_data = {f"radiant_gold_at_{kpi_minute_mark}m": 0, f"dire_gold_at_{kpi_minute_mark}m": 0, 
        ...
        for team_name_string, players_in_lane_for_team in teams_in_this_lane.items():
            ...
            opposing_laners_in_this_lane_objects = lanes_player_assignments[lane_name_key].get(opposing_team_string, [])
        ...
        num_radiant_laners = len(teams_in_this_lane.get("radiant",[])); num_dire_laners = len(teams_in_this_lane.get("dire",[]))
...
def set_player_name(account_id: int = typer.Argument(..., help="The player's unique Account ID."), custom_name: str = typer.Argument(..., help="The custom name to assign (e.g., 'SumaiL', 'Miracle-').")):
    ...
    try:
        db_connection.execute("INSERT OR REPLACE INTO player_custom_names (account_id, custom_name, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)", (account_id, custom_name))
        db_connection.commit()
...
def list_custom_names():
    ...
    try: 
        ...
        console.print(Panel(f"[bold red]SQLite query error for custom names:[/bold red]\n{error}", title="[bold red]DB Error[/bold red]", border_style="red")); db_connection.close(); raise typer.Exit(code=1)
    finally: db_connection.close()

    if not name_rows:
        console.print(Padding("[yellow]No custom player names found in the database.[/yellow]", (1,2))); return
...
def delete_player_name(identifier_to_delete: str = typer.Argument(..., help="The Account ID or Custom Name to delete.")):
    ...
    try: account_id_to_delete = int(identifier_to_delete)
    except ValueError:
        try:
            ...
            if name_row: account_id_to_delete = name_row["account_id"]
            else: console.print(f"[yellow]No custom name '{identifier_to_delete}' found.[/yellow]"); db_connection.close(); return
        except sqlite3.Error as error:
            console.print(Panel(f"[bold red]SQLite error finding custom name '{identifier_to_delete}':[/bold red]\n{error}", title="[bold red]DB Error[/bold red]", border_style="red")); db_connection.close(); return

    if account_id_to_delete is None:
        console.print(f"[yellow]Could not resolve '{identifier_to_delete}' to an Account ID for deletion.[/yellow]"); db_connection.close(); return
    try:
        cursor = db_connection.execute("DELETE FROM player_custom_names WHERE account_id = ?", (account_id_to_delete,))
        db_connection.commit()
        if cursor.rowcount > 0: console.print(f"[green]Successfully deleted custom name mapping for Account ID {account_id_to_delete}.[/green]")
        else: console.print(f"[yellow]No custom name found for Account ID {account_id_to_delete} to delete.[/yellow]")
    except sqlite3.Error as error:
...
def list_heroes():
    ...
    try:
        ...
        db_connection.close(); raise typer.Exit(code=1)
    finally: db_connection.close()

    if not heroes_rows_data:
        console.print(Padding("[yellow]No heroes found in the database. (Hint: Populate using the main data fetching script)[/yellow]", (1,2))); return
...
def list_matches(
    ...
    if not match_rows_data:
        ...
        console.print(Padding(message, (1,2))); db_connection.close(); return
    ...
    for match_row_item in match_rows_data:
        match_id = match_row_item["match_id"]; fetched_at_timestamp = match_row_item["fetched_at"]
        try:
        ...
        radiant_score = str(match_json_data.get('radiant_score', '-')); dire_score_val = str(match_json_data.get('dire_score', '-'))
        ...
        for player_info_item in players_in_match_data:
            ...
            player_detail_string = f"{player_identifier_string} ([hero]{hero_name_display_string}[/hero])" # Using a custom tag for hero for potential future styling
            ...
            if is_radiant_player_flag is None: 
                ...
                else: is_radiant_player_flag = False # Default assumption if no slot/isRadiant
    ...
    if matches_displayed_count > 0:
        ...
        if limit_matches and matches_displayed_count >= limit_matches : console.print(f"[dim]Showing up to {limit_matches} matches. Use --limit to change.[/dim]")
        elif search_team_name: console.print(f"[dim]Showing matches filtered by team: '{search_team_name}'[/dim]")
...
def find_player_hero_matches(
    ...
    try:
        ...
        for account_id_val, custom_name_val in player_custom_names_map.items():
            if player_identifier_argument.lower() == custom_name_val.lower():
                ...
                found_in_custom_names = True; break
    ...
    if not match_rows_data:
        console.print(Padding(f"[yellow]No matches in DB potentially involving '{player_identifier_argument}' based on initial SQL filter.[/yellow]", (1,2))); db_connection.close(); return

    found_matches_details_list = []
    for match_row_item in match_rows_data:
        try: match_json_data: Dict[str, Any] = json.loads(match_row_item["data"])
        ...
        for player_info_item in match_json_data.get("players", []):
            ...
            if search_target_account_id_value is not None: 
                ...
                if current_player_name and isinstance(current_player_name, str) and player_identifier_argument.lower() in current_player_name.lower():
                    player_matches_identifier_criteria_flag = True
            ...
            if player_matches_identifier_criteria_flag:
                ...
                if is_hero_search_active: 
                    if hero_played_by_target_player_id_in_this_match == target_hero_id_value:
                        player_found_in_match_and_met_criteria_flag = True; break 
                else: 
                    player_found_in_match_and_met_criteria_flag = True; break 
        ...
        if player_found_in_match_and_met_criteria_flag:
            ...
            radiant_score_val = str(match_json_data.get('radiant_score', '-')); dire_score_val = str(match_json_data.get('dire_score', '-'))
            winner_team_name = "N/A"
            if match_json_data.get('radiant_win') is True: winner_team_name = f"[bold bright_cyan]{radiant_team_name}[/bold bright_cyan]"
            elif match_json_data.get('radiant_win') is False: winner_team_name = f"[bold bright_red]{dire_team_name}[/bold bright_red]"
            ...
            if limit_matches_option and len(found_matches_details_list) >= limit_matches_option: break
    db_connection.close()
    ...
    if not found_matches_details_list:
        ...
        console.print(Padding(message, (1,2))); return

    table_title_string = f"Matches: Player '{display_search_term_string}'{table_title_suffix_text}"
    table = Table(title=table_title_string, show_header=True, header_style="bold magenta", border_style="dim purple", show_lines=True)
    table.add_column("Match ID", style="dim purple", width=12, justify="center")
    table.add_column("Player's Hero", style="hero", min_width=20) # Assuming 'hero' style is defined or defaults
    table.add_column("Radiant Team", style="bold bright_cyan", min_width=20)
    table.add_column("Dire Team", style="bold bright_red", min_width=20)
    table.add_column("Score (R-D)", justify="center", width=12, style="white")
    table.add_column("Winner", min_width=20, justify="center")
    ...
    if limit_matches_option and len(found_matches_details_list) >= limit_matches_option: console.print(f"[dim]Showing up to {limit_matches_option} matches. Use --limit to change.[/dim]")
...
def show_match_json(
    ...
    try:
        ...
        if db_connection: db_connection.close(); raise typer.Exit(code=1)
    ...
    try:
        ...
        if output_file_path:
            try:
                ...
                if output_directory and not os.path.exists(output_directory):
                    os.makedirs(output_directory); console.print(f"[info]Created directory: {output_directory}[/info]")
                with open(output_file_absolute_path, 'w') as file_writer: json.dump(match_json_parsed_data, file_writer, indent=2)
        ...
        if visualize_flag:
            ...
            if not VISUALIZATION_LIBRARIES_AVAILABLE: 
                ...
                if not visualization_was_successful: console.print("[yellow]Visualization generation failed or was skipped.[/yellow]")
            else: console.print("[yellow]Cannot visualize as JSON data was not parsed successfully.[/yellow]")
        
        should_print_to_console = True
        if output_file_path and output_file_absolute_path and os.path.exists(output_file_absolute_path): should_print_to_console = False
        if visualize_flag and visualization_was_successful: should_print_to_console = False
        
        if not should_print_to_console and ((visualize_flag and not visualization_was_successful) or (output_file_path and (not output_file_absolute_path or not os.path.exists(output_file_absolute_path)))):
        ...
        if output_file_path: 
            try:
                ...
                with open(output_file_absolute_path, 'w') as file_writer: file_writer.write(json_string_data_from_db)
...
def database_statistics():
    ...
    try:
        ...
        else: statistics_dictionary["Matches Fetched Between (UTC)"] = "N/A"
    except sqlite3.Error as error:
...
def player_summary(player_identifier_argument: str = typer.Argument(..., help="Player's Account ID, Custom Name, or current Persona Name.")):
    ...
    try:
        ...
        for account_id_val, custom_name_val in player_custom_names_map.items():
            if player_identifier_argument.lower() == custom_name_val.lower():
                ...
                resolved_by_custom_name_flag = True; break
        if not resolved_by_custom_name_flag:
            search_by_personaname_fallback_flag = True
            escaped_player_identifier = player_identifier_argument.replace('"', '""') # Basic escape for JSON search
    ...
    for match_row_item in match_rows_data:
        ...
        if player_in_this_match_info:
            ...
            if radiant_won_match_flag is not None and player_is_radiant_value is not None:
                if radiant_won_match_flag is True and player_is_radiant_value: wins_count += 1; player_won_this_match_flag = True
                elif radiant_won_match_flag is False and not player_is_radiant_value: wins_count += 1; player_won_this_match_flag = True

            if hero_id_value: 
                hero_performance_data[hero_id_value]["picks"] += 1
                if player_won_this_match_flag: hero_performance_data[hero_id_value]["wins"] += 1
    if db_connection: db_connection.close()

    if matches_played_count == 0:
        console.print(Padding(f"[yellow]No confirmed matches found for player '{display_player_term_string}' after detailed check.[/yellow]", (1,2))); return
    ...
    if hero_performance_data:
        ...
        top_heroes_table.add_column("Hero", style="hero") # Assuming 'hero' style
    ...
    for match_row_item in match_rows_data:
        ...
        if player_who_picked_hero_info:
            ...
            if radiant_won_match_flag is not None and picker_is_radiant_value is not None:
                if radiant_won_match_flag is True and picker_is_radiant_value: total_hero_wins_count += 1; picker_won_this_match_flag = True
                elif radiant_won_match_flag is False and not picker_is_radiant_value: total_hero_wins_count += 1; picker_won_this_match_flag = True
            ...
            if picker_won_this_match_flag: player_performance_with_hero_data[player_key_for_dict]["wins"] += 1
    if db_connection: db_connection.close()

    if total_hero_picks_count == 0:
        console.print(Padding(f"[yellow]Hero '{actual_hero_name_for_display}' (ID: {target_hero_id_value}) was not confirmed picked in any stored matches after detailed check.[/yellow]", (1,2))); return
...
def analyze_lanes_command(match_id_argument: int = typer.Argument(..., help="The Match ID to analyze.")):
    ...
    if not match_json_data_for_tables:
        console.print(Panel(f"[bold red]Could not re-fetch match data for console display for match {match_id_argument}.[/bold red]", title="[bold red]Data Error[/bold red]", border_style="red"))
        return
    ...
    for player_index, player_info_item in enumerate(players_data_for_tables_display):
        ...
        display_name = player_info_item.get("personaname", f"Slot {player_info_item.get('player_slot', player_index)}")
        if player_account_id and player_account_id in player_custom_names_map:
            display_name = player_custom_names_map[player_account_id]
    ...
    for lane_name_key, lane_information in analysis_data.get("lanes", {}).items():
        ...
        for team_name_string, players_in_lane_for_team_display in teams_in_lane_for_table_display.items():
            ...
            table = Table(
                ...
                title_style=f"bold {team_color_style}", show_header=True, header_style="bold magenta", border_style=f"dim {team_color_style}", show_lines=True
            )
            table.add_column("Player", style="white", min_width=15, overflow="fold")
            table.add_column("Hero", style="hero", min_width=15) # Assuming 'hero' style
            ...
            opposing_laners_hero_names_list_val = [p.get('hero_name_parsed', '') for p in lanes_assignments_for_tables_display[lane_name_key].get(opposing_team_string_value,[]) if p.get('hero_name_parsed')]
...
def export_match_ids(output_file_path_arg: str = typer.Option(..., "--output-file", "-o", help="Path to save the CSV file (e.g., all_match_ids.csv).", dir_okay=False, writable=True)):
    ...
    if not match_id_rows_data:
        console.print(Padding("[yellow]No match IDs found in the database to export.[/yellow]",(1,2))); return
    ...
    try:
        ...
        with open(output_file_absolute_path, 'w', newline='') as csv_file_writer:
            ...
            for row_item_data in match_id_rows_data: csv_writer_object.writerow([row_item_data["match_id"]])
...
def batch_analyze_lanes(
    ...
    try:
        with open(input_csv_file_path, 'r', newline='') as csv_file_reader:
            ...
            for row_item_data in csv_dict_reader:
                try: match_ids_to_process_list.append(int(row_item_data["match_id"]))
                except ValueError: console.print(f"[yellow]Warning: Skipping invalid match_id '{row_item_data['match_id']}' in '{input_csv_file_path}'.[/yellow]")
    ...
    if not match_ids_to_process_list:
        console.print(Padding(f"[yellow]No valid match IDs found in '{input_csv_file_path}' to process.[/yellow]", (1,2))); return
    ...
    output_directory = os.path.dirname(output_file_absolute_path)
    if output_directory and not os.path.exists(output_directory):
        try: os.makedirs(output_directory); console.print(f"[info]Created directory: {output_directory}[/info]")
    ...
    processed_ok_count = 0; error_count = 0
    ...
    if error_count > 0: console.print(f"[yellow]Encountered errors for {error_count} matches (details in CSV).[/yellow]")
...
def manage_kpi_parameters_config(
    ...
    for key, value in update_map.items():
        if value is not None: current_loaded_parameters[key] = value; parameters_were_changed = True
    ...
    if "score_weights" not in current_loaded_parameters or not isinstance(current_loaded_parameters["score_weights"], dict):
        current_loaded_parameters["score_weights"] = DEFAULT_KPI_PARAMETERS["score_weights"].copy(); parameters_were_changed = True
    
    score_weights_dict = current_loaded_parameters["score_weights"]
    for key, value in score_weights_update_map.items():
        if value is not None: score_weights_dict[key] = value; parameters_were_changed = True
    ...
    if show_current_config:
        console.print(Panel(f"Current Lane Analysis KPI Parameters (from [file_path]{CONFIG_FILE_NAME}[/file_path])", expand=False, border_style="blue", title_align="left", padding=(1,2)))
        ...
        else: weights_table.add_row("Score weights configuration is missing or invalid.", "")
